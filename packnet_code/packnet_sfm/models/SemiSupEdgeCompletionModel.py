...
class SemiSupEdgeCompletionModel(SelfSupModel):
    """
    ...
    kwargs : dict
        Extra parameters
    """
    def __init__(self, supervised_loss_weight=0.9, weight_rgbd=1.0, depth_edges_loss_weight=10.0, **kwargs):
    ...
    #def edge_loss(self, pred_depth, gt_edges, gt_mask=None):

    #    return self.edge_loss_head(pred_depth, gt_edges, gt_mask)
    ...
    def forward(self, batch, return_logs=False, progress=0.0, **kwargs):
        ...
        if not self.training:
            ...
            supervised_loss = (self.supervised_loss_weight * sup_output['loss'])/2
            ...
            if 'inv_depths_rgbd' in self_sup_output:
                ...
                edge_lidar_loss = self.compute_edge_loss_with_all_scales(self_sup_output['inv_depths_rgbd'],
                                                       batch, rgb_edge_mask,
                                                       is_grad=True, is_sigmoid=True, sigmoid_thresh=4)

                sup_output2 = self.supervised_loss(
                    self_sup_output['inv_depths_rgbd'], depth2inv(batch['depth']),
                    return_logs=return_logs, progress=progress)
                supervised_loss += (self.weight_rgbd * self.supervised_loss_weight * sup_output2['loss'])/2

                if 'depth_loss' in self_sup_output:
                    loss += self_sup_output['depth_loss']

            loss += supervised_loss
            loss += (edge_loss + edge_lidar_loss)/2
            # We use only top scale for edge loss. P.s. the first entry (0) includes 1-D vector - what is that? (TODO: check otherwise).
