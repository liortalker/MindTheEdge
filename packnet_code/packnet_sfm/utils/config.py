...
from packnet_code.packnet_sfm.utils.logging import s3_url, prepare_dataset_prefix
from packnet_code.packnet_sfm.utils.horovod import on_rank_0
from packnet_code.packnet_sfm.utils.types import is_cfg, is_list
from packnet_code.packnet_sfm.utils.misc import make_list
from packnet_code.packnet_sfm.utils.load import load_class, backwards_state_dict
...
def set_name(config):
    ...
    print('update path to be easily sorted by time')
    job_id = os.getenv('job_id') or os.getenv('LSB_JOBID')
    if config.name is not '':
        return '{}_{}_{}_{}'.format(
            datetime.now().strftime("%Y.%m.%d_%H%M"), config.name,
...
def set_checkpoint(config):
    ...
    if config.checkpoint.filepath is not '':
        ...
        config.checkpoint.yaml_path = config.config
    else:
...
def parse_train_file(file):
    ...
    # If it's a .ckpt checkpoint file
        ...
        return config, checkpoint
    elif 'contjob' in file:
        job_id = file[-6:]
        checkpoint_filepath = os.path.dirname(file)
        path_dirs = os.listdir(checkpoint_filepath)
        filter_dirs = [dir for dir in path_dirs if dir.endswith(job_id)]  # os.path.isdir(os.path.join(thedir, dir))
        assert len(filter_dirs) == 1, f'There are {len(filter_dirs)} folders with the same process_id number. cant resume train'
        list_of_files_in_dir = os.listdir(os.path.join(checkpoint_filepath, filter_dirs[0]))
        cp_files = sorted([file for file in list_of_files_in_dir if file.endswith('ckpt')])  # last is most trained
        assert len(cp_files) != 0, f'There is no ckpt files in folder: {filter_dirs}'
        latest_ckpt_file = os.path.join(checkpoint_filepath, filter_dirs[0], cp_files[-1])
...
def parse_train_config(cfg_default, cfg_file):
    ...
    support resume code from last checkpoint after LSF intervention
    Parameters
    ----------
    cfg_default : str
    ...
    # Check if should be resumed from other folder:
    job_id = os.getenv('job_id') or os.getenv('LSB_JOBID')
    path_dirs = os.listdir(config.checkpoint.filepath)
    filter_dirs = [dir for dir in path_dirs if dir.endswith(job_id)] # os.path.isdir(os.path.join(thedir, dir))
    assert len(filter_dirs)<2, 'There are more then one folder with the same process_id number. cant resume train'
    if len(filter_dirs)==0:
        # Return prepared configuration. Train New model
        return prepare_train_config(config), None
    # else len==1: reload from folder and resume the training
    list_of_files_in_dir = os.listdir(os.path.join(config.checkpoint.filepath, filter_dirs[0]))
    #yaml_file = [file for file in list_of_files_in_dir if file.endswith('yaml')][0] # assume only 1
    #yaml_file = os.path.join(config.checkpoint.filepath, filter_dirs[0], yaml_file)
    cp_files = sorted([file for file in list_of_files_in_dir if file.endswith('ckpt')]) # last is most trained
    if len(cp_files) == 0:
        yaml_files = [file for file in list_of_files_in_dir if file.endswith('yaml')]
        if len(yaml_files) == 1: # read the original yaml file from pid folder
            cfg_file = os.path.join(config.checkpoint.filepath, filter_dirs[0], yaml_files[0])
            config = get_default_config(cfg_default)
            config = merge_cfg_file(config, cfg_file)
        return prepare_train_config(config), None
    latest_ckpt_file = os.path.join(config.checkpoint.filepath, filter_dirs[0], cp_files[-1])
    return parse_train_file(latest_ckpt_file)
    # Loads default configuration
    #config_new = get_default_config(cfg_default)
    # Merge configuration file
    #config_new = merge_cfg_file(config_new, yaml_file)

    # Return prepared configuration
    #return prepare_resume_train_config(config_new, filter_dirs[0], last_cp_file)
...
def prepare_train_config(config):
    ...
    config.wandb.name = job_id = os.getenv('job_id') or os.getenv('LSB_JOBID')
    # Return configuration
    return config


# def parse_test_file(cfg_file):
#
#     assert cfg_file is None or cfg_file.endswith('yaml'), \
#         'You need to provide a .yaml file for configuration, not {}'.format(cfg_file)
#     cfg_default = 'packnet_code/configs/default_config'
#     cfg = parse_test_config(cfg_default, cfg_file)
#     return cfg
...
def parse_test_file(cfg_file):
    ...
    cfg_default = 'packnet_code/configs/default_config'
    f = open(cfg_file, 'r')
    tmp_cfg_file = CfgNode.load_cfg(f)
    f.close()
    cfg = parse_test_config(tmp_cfg_file.checkpoint.filepath, cfg_default, cfg_file)
    return cfg

# def parse_test_config(cfg_default, cfg_file):
#     """
#     Parse model configuration for testing
#
#     Parameters
...
#     config_from_file = merge_cfg_file(config_default, cfg_file)
#     # cfg = prepare_test_config(config)
#
#     ckpt_file = config_from_file.checkpoint.filepath
#
...
#         # # LT: bypass
#         if config_model.model.depth_net.name == 'PackNetSlimEnc01':
#             config_model.model.depth_net.name = 'PackNetSAN01'
#         # Override default configuration with model configuration
#         config = merge_cfgs(config_from_file, config_model)
#         # Update configuration for backwards compatibility
#         config = backwards_config(config)
#         config = merge_cfgs(config, config_from_file)
#     else:
...
def parse_test_config(ckpt_file, cfg_default, cfg_file):
    ...
    Parameters
    ----------
    ckpt_file : str, file path to load the ckpt
    cfg_default : str
        Default configuration file, with default values
    cfg_file : str
        Configuration file with updated information
    ...
    # Set pretrained model name
    config.save.pretrained = ckpt_file
    if config.save.folder is None:
        folder_path_to_save = os.path.dirname(config.checkpoint.filepath)
        folder_name_to_create = 'TEST_{}_{}_{}'.format(
            datetime.now().strftime("%Y.%m.%d_%H%M"),
            os.path.splitext(os.path.basename(config.checkpoint.filepath))[0][:8],
            os.path.splitext(os.path.basename(cfg_file))[0])
        config.save.folder = os.path.join(folder_path_to_save, folder_name_to_create)
    # Return prepared configuration and model state
