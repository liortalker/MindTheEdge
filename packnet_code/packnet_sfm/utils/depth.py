...
from packnet_code.packnet_sfm.utils.image import load_image, gradient_x, gradient_y, flip_lr, interpolate_image
from packnet_code.packnet_sfm.utils.types import is_seq, is_tensor
...
# Augment LIDAR map.
# Expects a scale_range of 2x3, where a column is (min, max) values for scale, and a row is for (i,j) pixel and value depth d
def augment_depth_values(sample, scale_range, add_range, drop_rate=0.1):

    # Return if depth value is None
    if sample is None:
        return sample

    # rows, cols, chan = sample.shape
    sample_shape = sample.shape
    rows = sample_shape[0]
    cols = sample_shape[1]

    scale_range = np.array(scale_range)
    add_range = np.array(add_range)

    idx_tuple = np.where(sample > 0)
    num_pts = len(idx_tuple[0])

    # PERTURB
    scale_val_i, add_val_i = rand_values_for_depth_augmentation(scale_range[:,0], add_range[:,0], num_pts, is_add_neg_allowed=True)
    scale_val_j, add_val_j = rand_values_for_depth_augmentation(scale_range[:,1], add_range[:,1], num_pts, is_add_neg_allowed=True)
    scale_val_d, add_val_d = rand_values_for_depth_augmentation(scale_range[:,2], add_range[:,2], num_pts, is_add_neg_allowed=False)


    dVec = sample[idx_tuple]
    # for d we take the scale to be global
    dVec = add_val_d + dVec * scale_val_d[0]

    iVec = idx_tuple[0] + add_val_i
    jVec = idx_tuple[1] + add_val_j
    iVec = np.round(iVec).astype('int')
    jVec = np.round(jVec).astype('int')

    # after pertrubing i,j it can be that we have collisions, filter them out
    # give unique number to each (i,j)
    running_idx_vec = iVec + rows*(jVec-1)
    sort_idx = np.argsort(running_idx_vec)
    diff_sorted_running_idx_vec = np.diff(running_idx_vec[sort_idx])
    idx_of_unique = np.where(diff_sorted_running_idx_vec != 0)[0]
    # add 1 because of diff
    idx_to_remain = sort_idx[idx_of_unique+1]
    sorted_idx_to_remain = np.sort(idx_to_remain)
    remained_running_idx_vec = running_idx_vec[sorted_idx_to_remain]
    remaind_dVec = dVec[sorted_idx_to_remain]
    remained_running_idx_vec_iVec = remained_running_idx_vec%rows
    remained_running_idx_vec_jVec = np.array([int(x) for x in (remained_running_idx_vec - remained_running_idx_vec_iVec)/rows + 1])

    # out of bounds indexes
    ok_indexes = (remained_running_idx_vec_iVec >= 0) * (remained_running_idx_vec_iVec < rows) * \
                 (remained_running_idx_vec_jVec >= 0) * (remained_running_idx_vec_jVec < cols)

    remained_running_idx_vec_iVec = remained_running_idx_vec_iVec[ok_indexes]
    remained_running_idx_vec_jVec = remained_running_idx_vec_jVec[ok_indexes]
    remaind_dVec = remaind_dVec[ok_indexes]

    # DROP: randomly drop points
    num_lidar_pts = len(remaind_dVec)
    num_to_drop = int(round(num_lidar_pts*drop_rate))
    random_perm = np.random.permutation(num_lidar_pts)
    random_lidar_pts_to_stay = np.sort(random_perm[:(num_lidar_pts-num_to_drop)])

    remained_running_idx_vec_iVec = remained_running_idx_vec_iVec[random_lidar_pts_to_stay]
    remained_running_idx_vec_jVec = remained_running_idx_vec_jVec[random_lidar_pts_to_stay]
    remaind_dVec = remaind_dVec[random_lidar_pts_to_stay]

    new_idx_tuple = (remained_running_idx_vec_iVec, remained_running_idx_vec_jVec)

    input_depth = np.zeros((rows,cols))
    input_depth[new_idx_tuple] = remaind_dVec
    input_depth = np.expand_dims(input_depth,2)

    sample = input_depth

    return sample

def rand_values_for_depth_augmentation(scale_range, add_range, num_pts, is_add_neg_allowed=True):

    if scale_range[0] is None or scale_range[1] is None:
        scale_val = 1
    else:
        scale_pos_or_neg = np.random.rand(num_pts)
        # scale_val = random.uniform(scale_range[0], scale_range[1])
        if scale_range[0] != scale_range[1]:
            scale_val = scale_range[0] + np.random.rand(num_pts) * (scale_range[1]-scale_range[0])
        else:
            scale_val = np.ones(num_pts)
        # if scale_pos_or_neg < 0.5:
        #     scale_val = 1 / scale_val
        scale_val[scale_pos_or_neg < 0.5] = 1 / scale_val[scale_pos_or_neg < 0.5]
    if add_range[0] is None or add_range[1] is None:
        add_val = np.zeros(num_pts)
    else:
        if add_range[0] != add_range[1]:
            add_val = add_range[0] + np.random.rand(num_pts) * (add_range[1] - add_range[0])
        else:
            add_val = np.zeros(num_pts)
        # add_val = random.uniform(add_range[0], add_range[1])
        if is_add_neg_allowed:
            scale_pos_or_neg = np.random.rand(num_pts)

            add_val[scale_pos_or_neg < 0.5] = -add_val[scale_pos_or_neg < 0.5]

    return scale_val, add_val