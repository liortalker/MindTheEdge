...
########################################################################################################################
    ...
    def _get_depth_file(image_file, depth_type):
        ...
        if depth_type == 'groundtruth':
            kitti_depth_prefix = '/mnt/algo-datasets/DB/KITTI_depth/train'
            depth_type_word = 'groundtruth'
        elif depth_type == 'velodyne':
            kitti_depth_prefix = '/mnt/algo-datasets/DB/KITTI_depth/data_depth_velodyne/train'
            depth_type_word = 'velodyne_raw'

        for cam in ['left', 'right']:
            if IMAGE_FOLDER[cam] in image_file:
                folder_name = '/'.join(image_file.split('sync')[0].split('/')[-1::]) + 'sync'
                depth_file = kitti_depth_prefix + '/' + folder_name + '/proj_depth/' + depth_type_word + '/' + IMAGE_FOLDER[cam] + '/' + image_file.split('/')[-1].split('\n')[0]
