...
from packnet_code.packnet_sfm.utils.misc import filter_dict
from packnet_code.packnet_sfm.utils.types import is_seq
...
def resize_sample_image_and_intrinsics(sample, shape,
    ...
    # Scale images
    for key in filter_dict(sample, [
        'rgb', 'rgb_original', 'target_rgb', 'target_rgb_original'
    ]):
        sample[key] = image_transform(sample[key])

    # edge_transform = transforms.Resize(shape, interpolation=image_interpolation)
    # for key in filter_dict(sample, [
    #     'edge', 'seg'
    # ]):
    for key in filter_dict(sample, [
        ...
        sample[key] = cv2.resize(sample[key], (out_w, out_h), cv2.INTER_NEAREST)
        # if key == 'edge':
        #     sample[key][sample[key] < 50] = 0
        #     sample[key][sample[key] >= 50] = 255
...
def resize_sample(sample, shape, image_interpolation=Image.ANTIALIAS):
    ...
    # Resize depth maps
    for key in filter_dict(sample, [
        'depth', 'input_depth', 'lidar', 'edge', 'rgb_edge', 'rgb_edge_for_loss',
        'target_edge', 'target_rgb_edge', 'target_rgb_edge_for_loss', 'target_depth', 'target_input_depth'
    ]):
        sample[key] = resize_depth_preserve(sample[key], shape)
        if key == 'edge' or key == 'rgb_edge' or key == 'rgb_edge_for_loss'\
                or key == 'target_edge' or key == 'target_rgb_edge' or key == 'target_rgb_edge_for_loss':
            if np.max(sample[key])>1:
                sample[key] = sample[key]/255

    for key in filter_dict(sample, ['normal']):
        sample[key] = cv2.resize(sample[key], (shape[1], shape[0]))

    for key in filter_dict(sample, [
        'edge_1', 'edge_2', 'edge_3'
    ]):
        key_scale = int(key.split('_')[-1])
        # cur_shape = (int(shape[0]/key_scale), int(shape[1]/key_scale))
        cur_shape = (int(shape[0] / (2 ** key_scale)), int(shape[1] / (2 ** key_scale)))
        sample[key] = resize_depth_preserve(sample[key], cur_shape)
        if key == 'edge_1' or key == 'edge_2' or key == 'edge_3':
            if np.max(sample[key]) > 1:
                sample[key] = sample[key]/255
    ...
    for key in filter_dict(sample, [
        ...
        key_scale = int(key.split('_')[-1])
        cur_shape = (int(shape[0]/(2**key_scale)), int(shape[1]/(2**key_scale)))
        sample[key] = cv2.resize(sample[key], (cur_shape[1], cur_shape[0]))
...
def to_tensor_sample(sample, tensor_type='torch.FloatTensor'):
    ...
    for key in filter_dict(sample, [
        'rgb', 'rgb_original', 'depth', 'input_depth', 'edge', 'rgb_edge', 'rgb_edge_for_loss',
        'edge_1', 'edge_2', 'edge_3', 'edge_4', 'edge_5', 'seg', 'lidar',
        'target_rgb', 'target_rgb_original', 'target_depth', 'target_input_depth',
        'target_edge', 'target_rgb_edge', 'target_rgb_edge_for_loss', 'normal',
        'normal_1', 'normal_2', 'normal_3'
    ]):
...
def crop_sample_input(sample, borders):
    ...
    # Crop images
    for key in filter_dict(sample, [
        'rgb', 'rgb_original', 'warped_rgb', 'target_rgb', 'target_rgb_original'
    ]):
    ...
    for key in filter_dict(sample, [
        'input_depth', 'bbox2d_depth', 'bbox3d_depth', 'rgb_edge', 'target_input_depth'
    ]):
...
def crop_sample_supervision(sample, borders):
    ...
    for key in filter_dict(sample, [
        ...
        'bwd_scene_flow', 'fwd_scene_flow', 'edge', 'lidar',
        'target_depth', 'normal'
    ]):
        sample[key] = crop_depth(sample[key], borders)
    # Crop context maps
