...
from packnet_code.packnet_sfm.utils.image import match_scales
from packnet_code.packnet_sfm.losses.loss_base import LossBase, ProgressiveScaling
...
########################################################################################################################
    ...
    def old_calculate_loss(self, inv_depths, gt_inv_depths):
        """
        ...
        if self.supervised_method.startswith('sparse'):
            for i in range(self.n):
                ...
                # inv_depths[i] = inv_depths[i]
                # gt_inv_depths[i] = gt_inv_depths[i]
                cur_loss_arr.append(self.loss_func(inv_depths[i], gt_inv_depths[i])*mask)
        # Return per-scale average loss
        return sum(cur_loss_arr) / self.n
    ...
    def calculate_loss(self, inv_depths, gt_inv_depths):
        ...
        # Return per-scale average loss
        # add epsilon due to some initial instability
        return sum([self.loss_func(inv_depths[i]+1e-5, gt_inv_depths[i])
                    for i in range(self.n)]) / self.n
